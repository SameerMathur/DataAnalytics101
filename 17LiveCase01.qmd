# Case: An Overview of the S&P500
...
1.  Suppose we wanted to list the stocks that have received a Rating of "Strong Sell".
2.  Recall that the following code will count the number of Stocks by Rating.

```{r, warning=FALSE, eval=TRUE, echo=TRUE}
table(sp500$Rating)
```

3.  Suppose we want to list the stocks that have "Strong Sell" rating. We can filter the stocks with a Rating of "Strong Sell"; select the columns: Stock, StockName, Price, GICSSector; arrange the resulting data by GICSSector. Accordingly, we can write the following code:
