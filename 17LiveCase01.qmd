# Live Case: Overview of the S&P500

**Chapter 17, *Last updated: Jan 05, 2024***
...
4.  Notice that the Google Sheet ID is: `14mUlNNpeuV2RouT9MKaAWKUpvjRijzQu40DdWJgyKPQ`.
...
The Global Industry Classification Standard (GICS^®^) was developed in 1999 by S&P Dow Jones Indices and MSCI. The GICS methodology aims to enhance the investment research and asset management process for financial professionals worldwide. The GICS methodology has been widely accepted as an industry analysis framework for investment research, portfolio management and asset allocation. Its universal approach to industries worldwide has contributed to transparency and efficiency in the investment process, and the GICS methodology supports the trend towards sector-based investing. \[4\]

The GICS classification consists of **11** sectors, **25** industry groups. The 11 sectors are {Communication Services, Consumer Discretionary, Consumer Staples, Energy, Financials, Health Care, Industrials, Information Technology, Materials, Real Estate, Utilities}. The classificaiton of each stock in the S&P 500 according to GICS is available at the following Google Sheet:

<https://docs.google.com/spreadsheets/d/1WrVA8dPYvQsc_mXVctgTntRLS02qd7ubzcdAsw03Lgk/edit#gid=0>

Again, we note that the Google Sheet ID is `1WrVA8dPYvQsc_mXVctgTntRLS02qd7ubzcdAsw03Lgk`

## Loading the data into R 

We will connect to both these Google Sheets and read the data into a tibble. We can use the function `gsheet2tbl` in package `gsheet` to read the Google Sheet into a tibble , as demonstrated in the following code.
...
sp500Data <- gsheet2tbl(url500) # Read it into a tibble called sp500
```

4.  **Reference**: This data has is current as of **`r sp500Data$Date[1]`**

Similarly, we can also read the GICS classification.
...
gics <- gsheet2tbl(urlgics) # Read it into a tibble called sp500
```

We join the two tibbles, using "Stock" as the key and name our joint tibble "sp500". For this purpose, we can write the following code.
...
# summary(sp500$`GICS Sector`)

sp500 %>%
  select(Stock, Description, GICSSector, Date, Price) %>%
  head(10) %>%
...
6.  The complete data has 39 columns. Our next goal is to gain a deeper understanding of what the data columns mean. We reorganize the column names into eight tables, labeled Table 1a, 1b.. 1h.
...
2.  We confirm that Rating is now modelled as a factor variable, by running the `str()` function. We can use the `levels()` function to review the different levels it can take.
...
-   The data showcases the S&P500 divided across `r length(unique(sp500$GICSSector))` Sectors. The S&P500 shares are divided into multiple Sectors. Each stock belongs to a unique sector. Thus, it makes sense to model Sector as a factor() variable.

```{r, warning=FALSE, eval=TRUE, echo=TRUE}
sp500$GICSSector <- as.factor(sp500$GICSSector)
```

2.  We confirm that `GICSSector` is now modelled as a factor variable, by running the `str()` function.
...
-   The S&P500 consists of `r nrow(sp500)` stocks, divided across `r length(unique(sp500$GICSSector))` sectors.
...
ggplot(data = sp500, 
  ...
  labs(title = "Stocks by GICS Sector", 
       x = "No. of Stocks", 
       y = "GICS Sector")
```

-   Thus, we can see how many stocks are part of each one of the 11 sectors. We can sum them to confirm that they add up to `r sum(table(sp500$GICSSector))` stocks.
...
2. Next, we create a row, to display the total across all GICS Sectors.

```{r}
# Create a row for the grand total
summary_row <- tibble(
  GICSSector = "Total",
  MarketCapBillions = sum(MarketCapbySector$MarketCapBillions)
)
summary_row
```

3. Finally, we append this row to the bottom of the summary table and display it.
...
# Render the table
MarketCapbySector %>%
  kable("html", caption = "Market Capitalization (Billions of USD) of S&P500 by GICS Sector") %>% 
  kable_styling()
```

4. This table gives us the breakdown of the Market Cap of the different stocks in the S&P500 across different GICS Sectors. We note that the Information Technology and Utilities sectors have the relatively highest and lowest Market Cap respectively. We also note that the entire S&P500 has a combined Market Cap of the S&P 500, as of `r unique(sp500$Date)`, is `r format(summary_row$MarketCapBillions, scientific = FALSE)` Billion USD. 

## Summary of Chapter -- Exploring S&P500 Data

This chapter explores the S&P500, a significant stock market index encompassing 500 major publicly traded companies in the U.S. The chapter introduces the index's role as a benchmark for assessing the overall health and performance of the U.S. stock market, maintained by S&P Dow Jones Indices.

This chapter guides readers through the intricacies of exploring S&P500 data, employing practical examples and R code to foster a deeper understanding of the breakdown of the S&P 500 across different GICS Sectors. Further exploration is encouraged with a wealth of references for continued learning and analysis.
